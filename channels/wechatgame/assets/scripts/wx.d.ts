 ...
 */
// Interactive feedback 交互反馈
interface ShowKeyboardOpts extends WxApiCallback {
...
interface KeyboardAPIs {
  showKeyboard: (options: ShowKeyboardOpts) => void;
  hideKeyboard: ZeroParamVoidFunc;
}
...
interface getGroupCloudOpts extends WxApiCallback<{ data: UserGameData[] }> {
  shareTicket: string;
  keyList: string[];
}

interface getFriendCloudOpts extends WxApiCallback<{ data: UserGameData[] }> {
  keyList: string[];
}

interface getUserCloudOpts extends WxApiCallback<{ KVDataList: KVData[] }> {
  keyList: string[];
}

interface setUserCloudOpts extends WxApiCallback {
  KVDataList: KVData[];
}

interface removeUserCloudOpts extends WxApiCallback {
  keyList: string[];
}
...
interface OpenDataContextAPIs {
  getSharedCanvas(): Canvas;
  getOpenDataContext: () => OpenDataContext;
  onMessage: (message: (data: Object)=> void) => void;
  getFriendCloudStorage: (options: getFriendCloudOpts) => void;
  getGroupCloudStorage: (options: getGroupCloudOpts) => void;
  getUserCloudStorage: (options: getUserCloudOpts) => void;
  removeUserCloudStorage: (options: removeUserCloudOpts) => void;
  setUserCloudStorage: (options: setUserCloudOpts) => void;
}
...
interface showAppMessageOpts extends WxApiCallback {
  title?: string;
  imageUrl?: string;
  query?: string;
}
...
interface OpenInterfaceAPIs {
  ...
  shareAppMessage: (options: showAppMessageOpts) => void;
  ...
  getLaunchOptionsSync: () => ApplaunchShowOpts;//TODO
}
